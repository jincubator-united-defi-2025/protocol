...
contract TychoRouterSingleSwapTest is TychoRouterTestSetup {
    function testSingleSwapPermit2() public {
        ...
        tychoRouter.singleSwapPermit2(
            amountIn, WETH_ADDR, DAI_ADDR, 2008817438608734439722, false, false, ALICE, permitSingle, signature, swap
        );
    ...
    function testSingleSwapUndefinedMinAmount() public {
        ...
        bytes memory protocolData =
            encodeUniswapV2Swap(WETH_ADDR, WETH_DAI_POOL, ALICE, false, RestrictTransferFrom.TransferType.None);

        bytes memory swap = encodeSingleSwap(address(usv2Executor), protocolData);

        vm.expectRevert(TychoRouter__UndefinedMinAmountOut.selector);
        tychoRouter.singleSwap(amountIn, WETH_ADDR, DAI_ADDR, 0, false, false, ALICE, true, swap);
    }
    ...
    function testSingleSwapWrapETH() public {
        ...
        IAllowanceTransfer.PermitSingle memory emptyPermitSingle = IAllowanceTransfer.PermitSingle({
        ...
        uint256 amountOut = tychoRouter.singleSwapPermit2{value: amountIn}(
            amountIn, address(0), DAI_ADDR, 1000_000000, true, false, ALICE, emptyPermitSingle, "", swap
        );
        uint256 expectedAmount = 2018817438608734439722;
        assertEq(amountOut, expectedAmount);
        uint256 daiBalance = IERC20(DAI_ADDR).balanceOf(ALICE);
    ...
    function testSingleSwapUnwrapETH() public {
        ...
        bytes memory protocolData = encodeUniswapV2Swap(
            DAI_ADDR, WETH_DAI_POOL, tychoRouterAddr, true, RestrictTransferFrom.TransferType.TransferFrom
        );

        bytes memory swap = encodeSingleSwap(address(usv2Executor), protocolData);

        uint256 amountOut = tychoRouter.singleSwapPermit2(
            amountIn, DAI_ADDR, address(0), 1000_000000, false, true, ALICE, permitSingle, signature, swap
        );
    ...
    function testSingleSwapNoTransferNeededIllegalTransfer() public {
        ...
        tychoRouter.singleSwap(amountIn, WETH_ADDR, DAI_ADDR, 2000 * 1e18, false, false, ALICE, false, swap);
    ...
    function testSingleSwapIntegrationPermit2() public {
        ...
        bytes memory callData = loadCallDataFromFile("test_single_swap_strategy_encoder");
        (bool success,) = tychoRouterAddr.call(callData);
    ...
    function testSingleSwapWithWrapIntegration() public {
        ...
        // Approve permit2
        vm.startPrank(ALICE);
        bytes memory callData = loadCallDataFromFile("test_single_swap_strategy_encoder_wrap");
    ...
    function testSingleSwapWithUnwrapIntegration() public {
        ...
        bytes memory callData = loadCallDataFromFile("test_single_swap_strategy_encoder_unwrap");
        (bool success,) = tychoRouterAddr.call(callData);
    ...
    function testSingleSwapIntegrationNoTransferIn() public {
        ...
        vm.startPrank(ALICE);
        bytes memory callData = loadCallDataFromFile("test_single_swap_strategy_encoder_no_transfer_in");
        (bool success,) = tychoRouterAddr.call(callData);
