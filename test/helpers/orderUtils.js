...
function buildMakerTraits({
  ...
  assert(
    BigInt(expiry) >= 0n && BigInt(expiry) < 1n << 40n,
    "Expiry should be less than 40 bits"
  );
  assert(
    BigInt(nonce) >= 0 && BigInt(nonce) < 1n << 40n,
    "Nonce should be less than 40 bits"
  );
  assert(
    BigInt(series) >= 0 && BigInt(series) < 1n << 40n,
    "Series should be less than 40 bits"
  );
...
function buildFeeTakerExtensions({
  ...
  return {
    ...
    postInteraction: ethers.solidityPacked(
      ...
      [
        feeTaker,
        makerReceiver ? "0x01" : "0x00",
        integratorFeeRecipient,
        protocolFeeRecipient,
      ].concat(makerReceiver ? [makerReceiver] : [], [
        integratorFee,
        integratorShare,
        resolverFee,
        whitelistDiscount,
...
function buildOrderRFQ(
  ...
  makerTraits =
    "0x" +
    setn(BigInt(makerTraits), _ALLOW_MULTIPLE_FILLS_FLAG, false)
      .toString(16)
      .padStart(64, "0");
  makerTraits =
    "0x" +
    setn(BigInt(makerTraits), _NO_PARTIAL_FILLS_FLAG, false)
      .toString(16)
      .padStart(64, "0");
  makerTraits =
    "0x" +
    setn(BigInt(makerTraits), _NEED_EPOCH_CHECK_FLAG, false)
      .toString(16)
      .padStart(64, "0");
...
function buildOrder(
  ...
  const allInteractionsConcat =
    allInteractions.map(trim0x).join("") + trim0x(customData);
...
function fillWithMakingAmount(amount) {
  return (
    BigInt(amount) | BigInt(buildTakerTraits({ makingAmount: true }).traits)
  );
}
...
function skipMakerPermit(amount) {
  return (
    BigInt(amount) | BigInt(buildTakerTraits({ skipMakerPermit: true }).traits)
  );
}
